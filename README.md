# Prerequisite:
- .net 8

# Build:
- use terminal to build (You can use visual studio if you love some UI click click instead)
```
cd .\StupidRobot\
dotnet build
```

# Run:
```
cd .\bin\Debug\net8.0
dotnet .\StupidRobot.dll
```

# Available commands: 
- PLACE X,Y,F -> set location of FRIDAY
- MOVE -> move FRIDAY forward
- LEFT -> turn FRIDAY left
- RIGHT -> turn FRIDAY right
- REPORT -> FRIDAY will tell you where she is
- EXIT (new) -> terminate FRIDAY :(

> [!TIP]
> If you lazy enough, you can download this [build version](https://github.com/nhanh9d/rms-robot/actions/runs/21235638091/artifacts/5214340787): 

> [!TIP]
> Have fun